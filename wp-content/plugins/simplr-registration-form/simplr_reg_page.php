...
function simplr_build_form($data,$atts) {
	...
	if($pro_fields) {
		foreach($pro_fields as $field) {
				if($field[name] != '') {
			$form .= '<div class="simplr-field"><label for="' .$field[name] .'" class="left">'.$field[label] .'</label>
<input type="checkbox" name="'.$field[name] .'" value="'.esc_attr($data[$field[name]]).'" class="field checkbox" /></div>';
